<?php
    ...
    $query = 'SELECT knjiga.*,  autor.ime AS ime_autora,
                                autor.prezime AS prezime_autora
                                FROM knjiga
                                INNER JOIN autor
                                ON knjiga.autor_id = autor.autor_id
                                ORDER BY knjiga.knjiga_id ASC';
    ...
    */
    ...
    /* Drugi nacin - Kopiranje niza i indeksiranje
    ...
    for($i = 0; $i < count($autori); $i++) {
        $numerisaniAutori[$autori[$i]->autor_id] = $autori[$i];
    }

    unset($autori);
     */

    /* Treci nacin - indeksiranje asocijativnog niza ID-em autora */
    $query = 'SELECT * FROM knjiga';
    $result = mysqli_query($database, $query);
    ...
    while($row = mysqli_fetch_object($result)) {
        $indeksiraniAutori[$row->autor_id] = $row;
    }
?>
<!doctype html>
<html>
        ...
        <table border="1">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <!-- <td><a href="izmenaAutora.php?autorId=<?php // echo $knjiga->autor_id; ?>"><?php // echo "$knjiga->ime_autora $knjiga->prezime_autora"; ?></a></td> -->
                    <td>
                        <a href="izmenaAutora.php?autorId=<?php echo $knjiga->autor_id; ?>">
                            <?php echo $indeksiraniAutori[$knjiga->autor_id]->ime . ' ' . $indeksiraniAutori[$knjiga->autor_id]->prezime; ?>
                        </a>
                    </td>
